...
import workerPolyfillsSource from './polyfills.js?raw'
import './PdfViewer.css'
import { metaphone, PHONETIC_SYMBOLS } from './phonetics'
import { printPdf } from './print'

// polyfills.js is only imported by main.jsx, which patches the main thread's
// Uint8Array. pdf.js does its parsing inside pdf.worker.min.mjs, which runs in
// its own module Worker global scope with its own Uint8Array - the main
// thread's patch never reaches it. So wrap the real worker script in a blob
// that applies the same polyfill first, then dynamically imports it.
const resolvedWorkerUrl = new URL(workerUrl, document.baseURI).href
const workerBootstrap = `${workerPolyfillsSource}\nawait import(${JSON.stringify(resolvedWorkerUrl)});\n`
const wrappedWorkerUrl = URL.createObjectURL(new Blob([workerBootstrap], { type: 'text/javascript' }))

pdfjsLib.GlobalWorkerOptions.workerSrc = wrappedWorkerUrl

// Under Electron the iframe is loaded via file://, where the browser's
// fetch()/XHR can't reliably read local files from a nested frame. Instead,
// ask the parent (ndp3) frame to read the bytes via its Node/IPC bridge.
function requestPdfFromElectron(filename) {
  return new Promise((resolve, reject) => {
    ...
    const onMessage = (event) => {
      ...
      if (msg.type === 'ndp3-pdf-viewer-pdf-data') {
        window.removeEventListener('message', onMessage)
        resolve(msg.data)
      } else if (msg.type === 'ndp3-pdf-viewer-pdf-error') {
        window.removeEventListener('message', onMessage)
        reject(new Error(msg.message))
      }
    }
    window.addEventListener('message', onMessage)
    window.parent.postMessage({ type: 'ndp3-pdf-viewer-request-pdf', requestId, filename }, window.location.origin)
  })
}

function loadPdfSource(url, isElectron) {
  if (!isElectron) return Promise.resolve(url)
  const filename = url.split('/').pop()
  return requestPdfFromElectron(filename).then((data) => ({ data }))
}
...
export default function PdfViewer({ url, title, onClose, docOptions, onSelectDoc, isElectron }) {
  ...
  useEffect(() => {
    let cancelled = false
    loadPdfSource(url, isElectron).then((source) => pdfjsLib.getDocument(source).promise).then(doc => {
      if (cancelled) return
      setPdfDoc(doc)
    ...
    <div className="pdf-viewer">
      <div className="pdf-toolbar">
        ...
        {docOptions && docOptions.length > 1 && (
          ...
          >
            {docOptions.map((doc) => (
              <option key={doc.url} value={doc.url}>{doc.label}</option>
            ))}
          </select>
        )}
